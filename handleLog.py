...
#        self.fileName = 'log-'+host+'.txt'
    ...
    def sendNext(self, hostSource, hostDest, expr):
        self.logFile = open(self.fileName, 'ab+')
        msg = [time.ctime()," - Servidor <",hostSource,"> enviou expressao: ", expr," para o host <",hostDest, ">\n"]
        self.logFile.write("".join(msg))
        self.logFile.close()

    def receiveResult(self, hostSource, hostDest, res):
        self.logFile = open(self.fileName, 'ab+')
        msg = [time.ctime(), " - Host <", hostDest, "> recebeu o resultado ", res, " do servidor <", hostSource, ">\n"]
        self.logFile.write("".join(msg))
        self.logFile.close()

    def receiveExpr(self, hostSource, hostDest, res):
        self.logFile = open(self.fileName, 'ab+')
        msg = [time.ctime(), " - Host <", hostDest, "> recebeu expressao: ", res, " do servidor <", hostSource, ">\n"]
        self.logFile.write("".join(msg))
        self.logFile.close()
    ...
    def sendResult(self, hostSource, hostDest, res):
        self.logFile = open(self.fileName, 'ab+')
        msg = [time.ctime(), " - Host <", hostSource, "> enviou o resultado ", res, " para o servidor <", hostDest, ">\n"]
        self.logFile.write("".join(msg))
        self.logFile.close()

    def sendClient(self, hostSource, hostDest, res):
        self.logFile = open(self.fileName, 'ab+')
